  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex h-full flex-col justify-end p-6">
        {item.tag && (
          <span className="text-accent font-medium text-xs tracking-wider uppercase mb-2 block">
            {item.tag}
          </span>
        )}
        ...
        {item.ctaLabel && item.href && (
          <a 
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 text-white hover:text-accent transition-colors text-sm font-semibold mt-auto"
            onClick={(e) => e.stopPropagation()}
          >
            {item.ctaLabel} <SquareArrowOutUpRight size={16} />
          </a>
        )}
      </div>
    </div>
